...
object List { // `List` companion object. Contains functions for creating and working with lists.
  ...
  def drop[A](l: List[A], n: Int): List[A] = l match {
    case Cons(head, tail) =>
      if (n > 0) drop(tail, n - 1)
      else l
    case Nil => Nil
  }

  //EXERCISE 4: Implement dropWhile,10 which removes elements from the List prefix as long as they match a predicate. Again, notice these functions take time proportional only to the number of elements being dropped—we do not need to make a copy of the entire List.
  ...
  def setHead[A](head:A, l: List[A]): List[A] = Cons(head, l)
  ...
  def product3(l: List[Double]) = foldLeft(l, 1D)(_*_)

  def reverse[A](l: List[A]): List[A] = foldLeft(l, Nil:List[A])((t, h)=> Cons(h,t))

  def filter[A](l: List[A])(f: A => Boolean): List[A] = foldRight(l,Nil:List[A])((a,acc)=> {
    if(f(a)) Cons(a, acc)
    else acc
  })
